...
public class CAddDeathsHandler implements IHostHttpHandler {

    @Override
    public String getContextPath() {
        return Config.getConfig().getAddDeathsContextPath();
    }
    ...
    public void handle(HttpExchange exchange) throws IOException {
        ...
        var variableName = Config.getConfig().getPlayerDeathsVar().replace("${player}", playerUUID);
        var variableContent = SkriptUtil.getVar(variableName);

        if (variableContent != null) {
            if (variableContent instanceof Long deaths) {
                LoggingUtil.Log("deaths: " + deaths  + " -> " + deaths + 1);
                SkriptUtil.setVar(variableName, deaths + 1);
            }
        } else {
            LoggingUtil.Log("deaths: 0 -> 1");
